...
class _LoginScreenState extends State<LoginScreen> {
    ...
    try {
      ...
      if (token != null) {
        ...
        if (role == "Damkar" && idDamkar > 0) {
          await userProvider.putTokenDamkar(idDamkar, token);
        } else if ((role == "Komandan" || role == "Anggota") && idPolisi > 0) {
          await userProvider.putTokenPolisi(idPolisi, token);
        } else {
  ...
  void _login() async {
    ...
    if (email.isEmpty || password.isEmpty) {
      final userProvider = Provider.of<UserProvider>(context, listen: false);
      userProvider.setErrorMessage("Email dan password tidak boleh kosong.");
      return;
    }
    ...
    if (userProvider.user != null) {
      ...
      await _getFCMTokenAndSendToServer(
        idDamkar: user.idDamkar,
        idPolisi: user.idPolisi,
        role: user.role,
      );
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => HomePage(user: user.toJson()),
        ),
      );
    } else {
      setState(() {}); // To refresh UI for error message shown by provider
    }
  }
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    focusedBorder: OutlineInputBorder(
                      borderSide:
                          BorderSide(color: Color(0xFFA1BED6), width: 2.0),
                    ),
                  ),
                ),
                const SizedBox(height: 10),
                TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    focusedBorder: OutlineInputBorder(
                      borderSide:
                          BorderSide(color: Color(0xFFA1BED6), width: 1.5),
                    ),
                  ),
                ),
                const SizedBox(height: 20),
                    ...
                    text: TextSpan(
                      ...
                      children: <TextSpan>[
                        TextSpan(
                          text: 'Sign Up',
                          style:
                              const TextStyle(color: Color(0xFFA1BED6)),
                          recognizer: TapGestureRecognizer()
                            ..onTap = () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (context) => const ModeApp()),
                              );
                            },
                        ),
                      ],
